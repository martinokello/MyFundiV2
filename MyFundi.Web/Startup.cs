...
namespace MyFundi.Web
{
    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            {
                var urlDev = "https://localhost:44339";
                var urlLive = "https://myfundi.martinlayooinc.com";
                if (env.IsProduction())
                {
                    spaBuilder.UseProxyToSpaDevelopmentServer(baseUri: urlLive);
                }
                else
                {
                    spaBuilder.UseProxyToSpaDevelopmentServer(baseUri: urlDev);
                }

                var path = Path.Combine(Directory.GetCurrentDirectory(), "ClientApp");
                spaBuilder.Options.SourcePath = path;
            });
        }
    }
